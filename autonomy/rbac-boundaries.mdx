...
The most important security property of autonomous remediation is architectural, not procedural: **ChangeGuard AI's control plane has no credentials for your cluster.** All execution authority lives inside your cluster, in an executor you install, bounded by Kubernetes RBAC that you grant.

## How execution reaches your cluster

```mermaid
flowchart LR
    subgraph saas [ChangeGuard AI control plane]
...
- The **executor** runs in your cluster and polls the control plane outbound over HTTPS, authenticating with a scoped API key. There is no inbound path: ChangeGuard AI cannot connect into your cluster, and there is no kubeconfig, token, or cloud credential for your cluster stored on our side.
...
Every one of these is under your control, effective immediately, with no ChangeGuard AI involvement:
