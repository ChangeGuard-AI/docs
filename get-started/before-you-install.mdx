...
| Collector | `changeguard-agent` | none set | none set | none |
| Optional scanners | `changeguard-agent` | per-job | per-job | none |

<Note>
  The collector Deployment does not currently set CPU/memory requests or limits — it runs as a single lightweight pod (a ~13MB image). If you enforce resource requests by policy (a `LimitRange`, Gatekeeper, etc.), account for that. Configurable collector resources are on the roadmap.
</Note>
