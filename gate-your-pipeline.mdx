...
If you turned on _…and what happened after each deploy_ when connecting the repository, the workflow also runs your deploy — **on a SHIP and only a SHIP** — and reports the outcome back to the same Change record, with nothing to add to the file. HOLD does not deploy: it means ChangeGuard could not establish that the change is safe, and treating "not forbidden" as "permitted" is the wrong default for the one step that touches production. Whether a failed check stops a merge is your branch-protection rule.
...
changeguard-check:
  ...
  script:
    - |
      ...
      test "$VERDICT" = "SHIP"          # only a SHIP deploys — see below for why HOLD does not
```
...
- **HOLD** — review first, and **do not deploy**. Most often ChangeGuard AI is missing evidence and says which: no environment named, the environment has not reported recently (stale), or the proposed artifact has not been independently evaluated. Missing evidence is never treated as safe, which is exactly why a HOLD must not reach production on the grounds that it was not a BLOCK.
