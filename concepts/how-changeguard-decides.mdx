...
**It knows** what the collector and scanners can observe: workloads and their state, every deployment, config, and GitOps change (including the diff that shipped), cluster events, RBAC posture, scan findings, and your incident history in ChangeGuard AI.
...
<Note>
  That last point is the accountability loop: ChangeGuard AI keeps score on itself. Prevented incidents, false positives, and missed failures are tracked per tenant from real outcomes — not self-reported.
</Note>
