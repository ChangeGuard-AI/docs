...
The collector is intentionally small (a single lightweight pod, a ~13MB image) and stateless — no PersistentVolumeClaims. ChangeGuard does not currently configure resource requests or limits for the collector by default; usage depends on cluster size (observed: ~9m CPU / 21Mi memory on a small cluster). If your organization enforces `LimitRange`s or quotas, account for the collector when defining them — and if you see resource pressure on a very large fleet, contact [support](mailto:support@changeguard.ai). Configurable resource settings are planned.
