...
<Warning>
  **The standalone agent has been removed.** ChangeGuard AI is now operator-managed — a single Helm chart installs the operator, the read-only data collector, and all five scanners. The commands on this page are historical and no longer work against the current installer.
